# Script Draft V2 — The Screenshot With No Date

## Beat 1 — Apparent certainty (Hook)
This screenshot is real. The repost is real. The conclusion people drew from it was wrong. Not because the image was fake — because the date was missing.

Look at this crop: `OpsStatus`, `Partial Service Disruption`, `Started: 02:14 UTC`, and `API Gateway: Degraded Performance.` On its face, that is all true. It also triggered panic when reposted on `2026-05-19 09:12 local` as proof the service was currently unstable.

The screenshot is not fabricated. The timeline context is missing.
...
When people get this wrong, it is usually not because they are irrational. It is because screenshots feel like direct evidence. We see concrete pixels and skip the time check.
...
A chart dip at `2026-04-06 03:00 UTC` shows `Core Checkout: 88.4%` and `Express Checkout: 87.9%`. Cropped tightly, that looks like a current collapse. Uncrop the top control and you see `Last 90 days · 2026-02-01 → 2026-04-30`. Keep scrubbing and both lines are back above `98.9%` by `2026-04-06 05:00 UTC`.
...
Before you share a screenshot, pause.

Open the original source page.
Check the time and timezone.
Scan for anything posted after the screenshot moment.
Then ask: “If I repost this now, am I describing the current state or a past state?”
...
**A screenshot can show a real fact from the wrong moment. Check the time before you share the story.**
